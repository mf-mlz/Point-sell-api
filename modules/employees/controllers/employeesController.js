...
const verificationToReset = async (req, res) => {
    ...
    if (!token || !password) { return res.status(400).json({ message: 'Token y Password son requeridos' }) };
    try {
        let tokenDecrypt = decryptCrypt(token);
        jwt.verify(tokenDecrypt, process.env.SECRET_NODE, { algorithms: ["HS256"] }, (err, decoded) => {
            if (!validateEmailAndId(decoded)) {
                return res.status(201).json({ message: 'Los datos del usuario no coinciden' });
            }
            else {
                if (!EditPs(decoded.id, password)) {
                    return res.status(500).json({ message: 'Ocurrio un error al editar la contraseña del usuario' });
                }
                else {
...
const EditPs = async (id, password) => {
    try {
        ...
        const updateEmployeesServices = await employeesService.putEmployeesPs(objEmp);
